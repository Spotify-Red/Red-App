...
class Root extends State<RootPage> {
  ...
  void initState() {
    super.initState();
    // Connect to Spotify Remote once when the widget initializes.
    connectToSpotifyRemote();

    // If a token is already stored (via hydrated bloc), trigger profile fetching.
    final spotifyAuthBloc = context.read<SpotifyAuthBloc>();
    if (spotifyAuthBloc.storedToken != null) {
      spotifyAuthBloc.add(FetchUserProfile());
    }
  }
            ...
            if (state is SpotifyProfileLoaded) {
              return Center(
                child: Column(
                  ...
                  children: [
                    ClipOval(
                      child: state.profile.image != 'Unknown'
                          ? Image.network(
                              state.profile.image,
                              fit: BoxFit.fill,
                              width: 200,
